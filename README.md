...
you ask Claude to do that task, it reads the instructions and follows them.
Think of it as a recipe card: clear steps that produce a consistent result
every time, no matter who asks.

Each skill lives in its own folder inside `skills/` and contains one file
called `SKILL.md`.
...
```
.
├── CLAUDE.md                         ← background Claude reads automatically
...
3. Claude finds the matching skill and follows it. You'll get a consistent,
   explained answer.

You don't have to name the skill — Claude picks the right one based on the
`description` line at the top of each `SKILL.md`. That's why a good
description matters (see below).

## Anatomy of a SKILL.md file

Open `skills/categorize-transaction/SKILL.md` and you'll see two parts.

**1. The header (between the `---` lines).** This tells Claude when to use
the skill:
...
description: >-
  ...
  belongs to. Use whenever someone pastes a transaction and asks how it
  should be booked.
---
```
...
- `description` — the most important line. Describe *what the skill does*
  and *when to use it*. Claude reads this to decide whether to use the
  skill, so be specific about the trigger ("use whenever someone...").

**2. The body (everything below).** Plain-English instructions: when to use
it, the steps to follow, and how to format the answer. Write it the way
you'd explain the task to a new colleague.

## How to add a new skill

No coding needed. Just:

1. **Copy** the `skills/categorize-transaction/` folder.
2. **Rename** the new folder to match the task, e.g.
   `skills/prepare-sales-invoice/`.
...
- Say what the skill should **not** do (e.g. "never post to the accounting
  system, only suggest").
- Tell Claude to **ask** when something is unclear instead of guessing.
- Test it: ask Claude to run the task with a real example and check the
